...
#include "node.hpp"
#include "pair.hpp"
#include "../iterator/set_red_black_iterator.hpp"
	...
	template <class T, class Compare, class Alloc, class Node = Node<T> >
		class Tree {
			...
			public:
				...
				typedef typename ft::set_Tree_iterator<Node>					iterator;
				typedef typename ft::const_set_Tree_iterator<Node>				const_iterator;
				...
				ft::pair<iterator, bool>insert(iterator	position, const value_type& new_data) {
					...
					while (x) {
						y = x;
						if (_cmp(x->data, new_data)) {
							if (x->parent && x == x->parent->left && (!_cmp(new_data, x->parent->data)))
								x = x->parent;
							else
								x = x->right;
						}
						else if (_cmp(new_data, x->data)) {
							if (x->parent && x == x->parent->right && (!_cmp(x->parent->data, new_data)))
								x = x->parent;
							else
								x = x->left;
						}
				...
				ft::pair<iterator, bool>	get_delete_node_ptr(const key_type &key) {
					...
					while (ptr && ptr->data != key) {
						if (_cmp(ptr->data, key))
							ptr = ptr->right;
						else if (_cmp(key, ptr->data))
							ptr = ptr->left;
						else
							break ;
					}
